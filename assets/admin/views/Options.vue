<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-semibold">Опции</h1>
      <button
        type="button"
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.id" class="border-t">
            <td class="px-4 py-2">
              <Input v-model="row.nameProxy" placeholder="Название опции" @blur="() => saveRow(row)" />
            </td>
            <td class="px-4 py-2">
          ...
          <tr v-if="!loading && rows.length === 0">
            <td colspan="3" class="px-4 py-6 text-center text-neutral-500">Нет опций</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <DialogRoot v-model:open="openCreate">
      <DialogPortal>
        ...
        >
          <div class="mb-2">
            <DialogTitle class="text-base font-semibold text-neutral-900 dark:text-neutral-100">Новая опция</DialogTitle>
          ...
          <form class="space-y-4" @submit.prevent="createSubmit">
            <Input v-model="createForm.name" label="Название" placeholder="Например: Размер" />
    ...
    <ConfirmDialog v-model="deleteOpen" :title="'Удалить опцию?'" :description="'Это действие необратимо'" confirm-text="Удалить" :danger="true" @confirm="performDelete" />
  </div>
...
const crud = useCrud<Option>(repo)
...
onMounted(async () => {
  await crud.fetchAll({ itemsPerPage: 500, sort: { sortOrder: 'asc', name: 'asc' } })
  syncRows()
})
...
function syncRows() {
  const items = ((state.items ?? []) as Option[]).slice()
  items.sort((a, b) => {
...
async function createSubmit() {
  ...
  try {
    ...
    Object.assign(createForm, { name: '', sortOrderStr: '' })
    publishToast('Опция добавлена')
  } finally {
